...
use crate::server::{get_server_config, Protocol, RequestCallback, HTTP1, HTTP2};
use crate::HTTP;
...
impl <'a>NewTcpServer {
    ...
    pub async fn on_request<'s, C, F>(&'s mut self, future: C) -> &mut Self
    where

        C: FnOnce(Request, Response) -> F + Send + Sync + 'a,
