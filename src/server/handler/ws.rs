    ...
    R: AsyncRead + AsyncWrite + Unpin + Send + Sync + 'a
{
    ...
    pub async fn handle(&mut self, mut req: Request, mut res: Response) -> Result<()> {
        // TODO: handshake...
        ...
        println!("{:?}", parse(&mut res).unwrap());

        self.writer
            .write(parse(&mut res).unwrap().as_bytes())
            .await
            .unwrap();
        ...
        // TODO: fix
        // if self.http.router().router.match_ws_routes(&mut req, &mut res).await.is_none() {
        //     return Ok(());
        // }
